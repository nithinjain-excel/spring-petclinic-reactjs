...
import { isLoggedIn, logout, getUsername } from '../util/auth';
      ...
      <div className='container'>
        ...
        <div className='navbar-collapse collapse' id='main-navbar'>
          <ul className='nav navbar-nav navbar-right'>
            ...
            {loggedIn ? (
              <li>
                <a onClick={handleLogout} style={{ cursor: 'pointer' }} title='logout'>
                  <span className='glyphicon glyphicon-log-out' aria-hidden='true'></span>&nbsp;
                  <span>Logout ({username})</span>
                </a>
              </li>
            ) : (
              <MenuItem active={name === '/login'} url='/login' title='login'>
